...
internal sealed class VirtualRelayFrontEndRuntime
{
    ...
    public MeasurementFrame Process(MeasurementFrame terminalMeasurement)
    {
        ...
        if (now >= _nextCaptureAt.Value)
        {
            ...
            // Advance from the configured ADC grid rather than from a late caller
            // timestamp. A short/non-uniform outer step may delay one actual capture,
            // but it must never permanently phase-shift every subsequent ADC sample.
            var next = _nextCaptureAt.Value + _profile.AdcSampleInterval;
            while (next <= now)
                next += _profile.AdcSampleInterval;
            _nextCaptureAt = next;
        }
        ...
        return _held is null
            ? UnavailableLike(terminalMeasurement, now)
    ...
    private static MeasurementFrame UnavailableLike(MeasurementFrame source, DateTimeOffset timestamp)
    {
        var unavailable = new SmvTrustState(
            false,
            false,
            false,
            "FRONTEND_SETTLING",
            "Relay analog front end has not yet released its first acquired measurement.");
        ...
        return new MeasurementFrame(timestamp, 0, 0, 0, 0, unavailable, zero);
    }
